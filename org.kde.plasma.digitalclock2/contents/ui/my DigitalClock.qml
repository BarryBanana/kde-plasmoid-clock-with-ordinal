...
MouseArea {
    ...
    states: [
        State {
            ...
            PropertyChanges {
                target: sizehelper
                height: Math.min(Plasmoid.configuration.showDate || timeZoneLabel.visible ? main.height * 0.56 : main.height * 0.71,fontHelper.font.pixelSize)
                font.pixelSize: sizehelper.height
            }
        },

        // HERE :  horizontal mode.
        State {
            name: "oneLineDate"
    ...
    */
    ...
    function timeFormatCorrection(timeFormatString = Qt.locale().timeFormat(Locale.ShortFormat)) {
        ...
        if (!match) {
            ...
            timeFormat = "H:mm"; // 12-hour format without seconds
            timeFormatWithSeconds = "H:mm"; // No seconds needed
            setupLabels();
            return;
        }
        ...
        // Use 12-hour format with no leading zeros, no seconds, and no AM/PM
        let result = "H" + delimiter + minutes;

        // Set both timeFormat and timeFormatWithSeconds to the same format (no seconds)
        timeFormat = result;
        timeFormatWithSeconds = result; // No seconds needed
        setupLabels();
    }
